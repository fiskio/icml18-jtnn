...
class JTNNEncoder(nn.Module):
    ...
    def __init__(self, vocab, hidden_size, embedding=None, use_cuda=False):
    ...
    def forward(self, root_batch):
        ...
        root_vecs = node_aggregate(root_batch, h, self.embedding, self.W, use_cuda=self.use_cuda)
...
def node_aggregate(nodes, h, embedding, W, use_cuda=False):
    x_idx = []
    h_nei = []
    hidden_size = embedding.embedding_dim
    padding = create_var(torch.zeros(hidden_size), False, use_cuda=use_cuda)
    ...
    x_vec = create_var(torch.LongTensor(x_idx), use_cuda=use_cuda)
